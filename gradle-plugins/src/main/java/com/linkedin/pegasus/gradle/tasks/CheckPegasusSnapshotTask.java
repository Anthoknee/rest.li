...
public class CheckPegasusSnapshotTask extends DefaultTask
{
  ...
  private boolean hasSchemaAnnotationHandler()
  {
    ...
    if (_handlerClassNames.size() != expectedHandlersNumber)
    {
      String errorMsg = String.format("Encountered errors when searching for annotation handlers: total %s dependencies configured, but %s handlers found: [%s].",
          ...
          _handlerClassNames.size(),
          String.join(",", _handlerClassNames));
      throw new GradleException("Error while getting schema annotation handlers to do annotation compatibility check: "  + errorMsg);
    }
    return true;
  }
}
